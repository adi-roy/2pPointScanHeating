...
Tsurface = params.therm.T_inf; %Fixed temperature at surface of water; convection is not simulated
...
if isfield(params, 'u_start')
    ...
    u(:,1:water_pixels) = Tsurface; %water temperature is roughly 30c
    glass_center = Tsurface;
%     glass_bottom = Tinit;
%     glass_axis = linspace(Tsurface,Tinit,glass_pixels-1);
%     u(1,water_pixels+1:water_pixels+1+glass_pixels) = [glass_center,glass_axis,glass_bottom];
    glass_edge = Tinit;
    glass_surface_top = linspace(glass_center,glass_edge,ceil(rg/u_step))';
%     glass_internal = ones(ceil(rg/u_step),glass_pixels);
%     for i = water_pixels+1:water_pixels+1+glass_pixels
%         glass_internal(i-water_pixels) = linspace(u(1,i)',Tinit,ceil(rg/u_step));
%     end
    u(1:ceil(rg/u_step),water_pixels+1) = [glass_surface_top];
%     u(:,1) = Tsurface;
%     u(end,1:water_pixels) = Tinit;
...
%         (k(1:m,[1 1:n-1])).*u(1:m,[1 1:n-1]))./(u_step.^2);
    ...
    vrr(1:m,1:n) = ((k(1:m,1:n).*(ri+u_step/2)).*u([2:m m],1:n) -(k(1:m,1:n).*(ri+u_step/2)+k([1 1:m-1],1:n).*(ri-u_step/2)).*u(1:m,1:n) +...
        (k([1 1:m-1],1:n).*(ri-u_step/2)).*u([1 1:m-1],1:n))./(u_step.^2*ri);
% %     vzz(1:m,1) = ((k(1:m,1)).*u(1:m,2) -(k(1:m,1)+k(1:m,1)).*u(1:m,1) +...
% %         (k(1:m,1)).*(2.*h.*u_step.*(T_inf-u(1:m,1))./k(1:m,1)+u(1:m,2)))./(u_step.^2);
% %     vzz(1:m,2:n) = ((k(1:m,2:n)).*u(1:m,[3:n n]) -(k(1:m,2:n)+k(1:m,[2 2:n-1])).*u(1:m,2:n) +...
% %          (k(1:m,[2 2:n-1])).*u(1:m,[2 2:n-1]))./(u_step.^2);
% 
    vzz(1:m,1:water_pixels) = ((k(1:m,1:water_pixels)).*u(1:m,[2:water_pixels water_pixels]) -(k(1:m,1:water_pixels)+k(1:m,[1 1:water_pixels-1])).*u(1:m,1:water_pixels) +...
        (k(1:m,[1 1:water_pixels-1])).*u(1:m,[1 1:water_pixels-1]))./(u_step.^2);
    vzz(1:m,water_pixels+1) = ((k(1:m,water_pixels+1)).*u(1:m,water_pixels+2) -(k(1:m,water_pixels+1)+k(1:m,water_pixels)).*u(1:m,water_pixels+1) +...
        (k(1:m,water_pixels)).*(2.*h.*u_step.*(T_inf-u(1:m,water_pixels+1))./k(1:m,water_pixels+1)+u(1:m,water_pixels+2)))./(u_step.^2);
    vzz(1:m,water_pixels+2:n) = ((k(1:m,water_pixels+2:n)).*u(1:m,[water_pixels+3:n n]) -(k(1:m,water_pixels+2:n)+k(1:m,[water_pixels+2 water_pixels+2:n-1])).*u(1:m,water_pixels+2:n) +...
         (k(1:m,[water_pixels+2 water_pixels+2:n-1])).*u(1:m,[water_pixels+1 water_pixels+2:n-1]))./(u_step.^2);
...
%     uchange1(ceil(2*rg/u_step):end,1)=0;
%     uchange1(end,water_pixels+1:end)=0; %body core temp at sides
    uchange1(end,1:end)=0; %temp at sides
    uchange1(:,end)=0; %clamp the temperature deep
