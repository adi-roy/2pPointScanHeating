function [T_space] = heat_laser_Robin(source, duty, cycle_time, p, cp, k, qm, T_incubation, water_px, glass_px, x_array, z_array, dt, dx, sim_time, id, i, T_space, hconv, T_inf)
...
%     T_temporal = zeros(length(z_array),sim_time);
    ...
    for t = dt:dt:sim_time
        ...
        urr(1:m,1:n) = ((k(1:m,1:n).*(xi+dx/2)).*T_mat([2:m m],1:n) -(k(1:m,1:n).*(xi+dx/2)+k([1 1:m-1],1:n).*(xi-dx/2)).*T_mat(1:m,1:n) +...
            (k([1 1:m-1],1:n).*(xi-dx/2)).*T_mat([1 1:m-1],1:n))./(dx.^2*xi);
        uzz(1:m,1:water_px) = ((k(1:m,1:water_px)).*T_mat(1:m,[2:water_px water_px]) -(k(1:m,1:water_px)+k(1:m,[1 1:water_px-1])).*T_mat(1:m,1:water_px) +...
            (k(1:m,[1 1:water_px-1])).*T_mat(1:m,[1 1:water_px-1]))./(dx.^2);
        uzz(1:m,water_px+1) = ((k(1:m,water_px+1)).*T_mat(1:m,water_px+2) -(k(1:m,water_px+1)+k(1:m,water_px)).*T_mat(1:m,water_px+1) +...
            (k(1:m,water_px)).*(2.*hconv.*dx.*(T_inf-T_mat(1:m,water_px+1))./k(1:m,water_px+1)+T_mat(1:m,water_px+2)))./(dx.^2);
        uzz(1:m,water_px+2:n) = ((k(1:m,water_px+2:n)).*T_mat(1:m,[water_px+3:n n]) -(k(1:m,water_px+2:n)+k(1:m,[water_px+2 water_px+2:n-1])).*T_mat(1:m,water_px+2:n) +...
             (k(1:m,[water_px+2 water_px+2:n-1])).*T_mat(1:m,[water_px+1 water_px+2:n-1]))./(dx.^2);
